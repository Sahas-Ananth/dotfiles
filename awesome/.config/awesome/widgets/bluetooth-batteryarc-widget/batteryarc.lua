...
local function worker(user_args)
  ...
  local function update_widget(widget, stdout)
    local charge = -1
    local status
    -- naughty.notify { title = stdout }
    for s in stdout:gmatch "[^\r\n]+" do
      local cur_charge = tonumber(s)
      if cur_charge ~= nil then
        charge = cur_charge
      end
    end
    ...
    if status == "Charging" then
      ...
      text_with_background.bg = "#0000CDFF" -- "#00000000"
  ...
  watch(
    [[bash -c "bluetoothctl info | tail -n 1 | sed 's/.*Bat.*(\([0-9]*\))/\1/'"]],
    timeout,
    update_widget,
    batteryarc_widget
  )
    ...
    awful.spawn.easy_async(
      ...
      function(stdout, _, _, _)
        ...
        notification = naughty.notify {
          text = stdout,
          title = "Bluetooth Battery status",
          timeout = 5,
          width = 300,
